...
# Относительный импорт оставляем — он будет работать, если вызывать как модуль
# Но для тестов мы будем передавать storage вручную
from . import storage

def generate_report(storage_instance):
    """
    Генерирует простой текстовый отчет по всем расходам.
    Используется в тестах.
    """
    expenses = storage_instance.get_all_expenses()
    if not expenses:
